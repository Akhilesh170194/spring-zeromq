...
public class ZmqTemplate implements DisposableBean {
    ...
    public ZmqTemplate(ZContext context) {
        this(context, 1000, new Jackson2JsonMessageConverter());
    }

    /**
     * Create a new template with the given context, buffer size, and a default JSON message converter.
     *
     * @param context    the ZeroMQ context
     * @param bufferSize the buffer size (high watermark) for sockets
     */
    public ZmqTemplate(ZContext context, int bufferSize) {
        this(context, bufferSize, new Jackson2JsonMessageConverter());
    }
     ...
     */
    public ZmqTemplate(ZContext context, int bufferSize, MessageConverter messageConverter) {
